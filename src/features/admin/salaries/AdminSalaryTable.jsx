  ...
  const handleCreateSalary = async (newSalaryData) => {
    try {
      ...
      if (response?.data?.data) {
        setSalaries((prevSalaries) => [...prevSalaries, response.data.data]);
        setShowModal(false);
      } else {
        console.error("Failed to create Salarie: No data returned from server");
      }
    } catch (error) {
      console.error(
    ...
    <Menus>
      <Modal>
        <Modal.Open opens="create-salary">
          <StyledButton onClick={toggleModal}>Create New Salary</StyledButton>
        </Modal.Open>
